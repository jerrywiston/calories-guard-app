...
export function renderMarkdown(report) {
  ...
  const lines = [
    ...
    `| 食物 | 備註份量 | 估計熟重 (g) | ${Object.values(NUTRIENTS).join(' | ')} | 信心 |`,
    `| ${Array(Object.keys(NUTRIENTS).length + 4).fill('---').join(' | ')} |`,
  ];
  for (const item of analysis.items) {
    lines.push(`| ${[item.name, item.portion_description || '—', item.estimated_weight_g, ...Object.keys(NUTRIENTS).map(key => item[key]), confidence[item.confidence]].map(cell).join(' | ')} |`);
  }
